...
Peripheral_polygon::Peripheral_polygon() = default;
Peripheral_polygon::~Peripheral_polygon() {
	LOG_INFO(LOG_HEADER(Message, "~RendererPeripheral()"), LOG_VALUE(frameFenceValue_[0]));
}

[[nodiscard]] bool Peripheral_polygon::initalize(RendererCore* core, windowBase* window, const int& buffercount) {
    ...
    LOG_INFO(LOG_HEADER(Message, "Peripheral_polygon SUCSSECED"));
    return true;
}

//---------------------------------------------------------------------------------------------------

void Peripheral_polygon::update() {
	...
	const float clearColor[] = { 1.0f, 1.0f, 0.0f, 1.0f };  // 黒でクリア
